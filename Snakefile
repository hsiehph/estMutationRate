...
rule identityCount:
	...
	priority: 10
	shell:
			""" file=`basename {input} .fasta`; seqID=`grep \">\" {input} | grep -P "{wildcards.id1}|{wildcards.id2}" || echo ""` ; numSeq=`echo ${{seqID}} | tr -cd '>' | wc -c` ; if [[ $numSeq -eq 2 ]]; then perl scripts/extractFASTAseqs.pl -n {dir_inFASTA}/{wildcards.FASTA} "${{seqID}}" > {output[0]} ; arr+=($(echo ${{seqID}} | sed 's/>//g' | sed 's/[:|-]/ /g')) ; mafft --maxiterate 100 --globalpair --adjustdirectionaccurately  {output[0]} > {output[1]} ; python scripts/calc_windowIdentity.v2.py --window 10000000000 --list_addInfo \"['${{arr[0]}}','${{arr[1]}}','${{arr[2]}}','${{arr[3]}}','${{arr[4]}}','${{arr[5]}}', '{wildcards.id1}-{wildcards.id2}']\" {output[1]} > {output[2]} ; else touch {output[0]} {output[1]} {output[2]} ;fi """
